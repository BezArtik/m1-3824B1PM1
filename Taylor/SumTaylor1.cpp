...
void CompareAndOutputResults(int FunctionType, int SummationMethod, float x, int count) {
    ...
    //МАССИВ ФУНКЦИЙ ДЛЯ ПОДСЧЕТА ЧЛЕНОВ РЯДА
    ...
    //МАССИВ ФУНКЦИЙ ДЛЯ СУММИРОВАНИЯ
    SummationFunction SummationMethods[] = {
        NULL,
        DirectSummation,      //ПРЯМОЕ СУММИРОВАНИЕ
        ReverseSummation,     //ОБРАТНОЕ СУММИРОВАНИЕ
        PairwiseSummationRes  //ПОПАРНОЕ СУММИРОВАНИЕ
    };

    //МАССИВ ФУНКЦИЙ ИЗ math.h
